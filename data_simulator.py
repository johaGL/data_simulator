...
python3 data_simulator.py --m 19 --m_with_overlap 10 --card_a 6 --card_b 7 \
      --value_min_whole_df 90 --value_max_whole_df 777 $OUTPUT_DIR
...
def simul_data_args():
    ...
    parser.add_argument("--value_min_whole_df", type=float,
                        help="minimal value for entire dataset")

    parser.add_argument("--value_max_whole_df", type=float,
...
def generate_ranges_tuples(value_min_whole_df, value_max_whole_df, m) -> list:
    """
    generates a list of tuples, each being a range
    example output : [(139.11, 1175.4), (85.042, 1085.22), (714.08, 1169.5), ...
    length is m
    """
    total_plage = value_max_whole_df - value_min_whole_df
    ...
    while len(out_l) < m:
        ...
        x_inf_extreme_possible = value_max_whole_df - span_list[k]
        random_initial_point = np.random.uniform(value_min_whole_df, x_inf_extreme_possible, 1)[0]
...
def partition_m_variables_2_groups(m: int) -> list:
    """ outputs a list, first element for Pos group, second element for Neg group """
    tmp_resid = m % 2
    m_by_group = int(m / 2)
    if tmp_resid != 0:  # when odd m, assign the residue to the Pos group
        m_list = [int(m_by_group + tmp_resid), int(m_by_group)]
    else:
        m_list = [int(m_by_group), int(m_by_group)]
...
def generate_values_df(grouped_ranges_dict, card_a, card_b):
    outl = []
    i = 1
...
if __name__ == "__main__":
    ...
    value_min_whole_df = args.value_min_whole_df
    value_max_whole_df = args.value_max_whole_df
    ...
    # value_min_whole_df = 65  # do not put zero ?
    # value_max_whole_df = 1110
    ...
    assert value_min_whole_df < value_max_whole_df, "Error, the min value is not inferior to the max"
    ...
    print(f"Generating simulated data:\
          ...
          and for the whole dataframe min,max: [{value_min_whole_df},{value_max_whole_df}]")

    n = card_a + card_b  # total number of subjects

    ranges_list = generate_ranges_tuples(value_min_whole_df, value_max_whole_df, m)

    #m_list = partition_m_variables_2_groups(m)
    ...
    df = generate_values_df(grouped_ranges_dict, card_a, card_b)
