"""
**Optional** script:
   generates metadata of a simulated data,
   if interested in running with the latest DIMet tool version
"""
import os
import numpy as np
import pandas as pd
...
if __name__ == "__main__":
    ...
    tabl = args.in_file
    df = pd.read_csv(tabl, sep='\t', index_col=0, header=0)
    ...
    file_out = os.path.join(args.dir_out,
                            tabl.split("/")[-1].replace(".tsv", ".csv").replace("data_", "meta_"))
    metada_df.to_csv(file_out, index=False, sep='\t')
    print("done")
